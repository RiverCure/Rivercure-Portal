...
class OrganizationCreateView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        membership = Membership(user=manager, organization=organization, access_granted=True,
                                access_grant_date=currentTime, permission='org_manager')
        membership.save()

        # Notify new manager
        notify.send(sender=organization, recipient=manager, action_object=organization,
                    verb=f"You have been assigned management of the new organization {organization.code} by {self.request.user}")
...
class MemberRoleUpdateView(UserPassesTestMixin, UpdateView):
    ...
    def get_success_url(self):
        ...
        managers = User.objects.filter(membership__in=Membership.objects.filter(
            organization=organization, permission='org_manager')).exclude(pk=self.request.user.id)
        notify.send(sender=organization, recipient=managers, action_object=organization,
                    verb=f"User {user} role on organization {organization.code} has been changed to {newRole} by {self.request.user}")
        # Notify user
        notify.send(sender=organization, recipient=user, action_object=organization,
                    verb=f"Your role on organization {organization.code} has been changed to {newRole} by {self.request.user}")

        return reverse('organization-manage', args=(self.get_object().organization.id,))

    def test_func(self):
        organization = self.get_object().organization
        cond1 = self.request.user in User.objects.filter(
            membership__in=Membership.objects.filter(organization=organization, permission='org_manager'))
        cond2 = Membership.objects.filter(organization=organization, permission='org_manager').count(
        ) == 1 and self.get_object().permission == 'org_manager' and is_platform_admin(self.request.user)
...
def organizationAccessRequestDeny(request, organizationId, userId):
    ...
    if membership_user and organization.is_active == True:
        ...
        # Notify the user
        notify.send(sender=organization, recipient=user, action_object=organization,
                    verb=f"You have been denied access to organization {organization.code}")
        # Notify all managers of the organization
        managers = User.objects.filter(membership__in=Membership.objects.filter(
            organization=organization, permission='org_manager')).exclude(pk=request.user.id)
        notify.send(sender=organization, recipient=managers, action_object=organization,
                    verb=f"User {user} has been denied access to organization {organization.code}")
...
def organizationAccessRemove(request, organizationId, userId):
    ...
    if membership_user and organization.is_active == True:
        ...
        notify.send(sender=organization, recipient=user, action_object=organization,
                    verb=f"Your access to organization {organization.code} has been removed")
        # Notify all managers of the organization
        managers = User.objects.filter(membership__in=Membership.objects.filter(
            organization=organization, permission='org_manager')).exclude(pk=request.user.id)
        notify.send(sender=organization, recipient=managers, action_object=organization,
                    verb=f"User {user} access to organization {organization.code} has been removed")
...
def organizationAccessAllow(request, organizationId, userId):
    ...
    if membership_user and organization.is_active == True:
        ...
        # Notify the user
        notify.send(sender=organization, recipient=user, action_object=organization,
                    verb=f"You have been granted access to organization {organization.code}")
        # Notify all managers of the organization
        managers = User.objects.filter(membership__in=Membership.objects.filter(
            organization=organization, permission='org_manager')).exclude(pk=request.user.id)
        notify.send(sender=organization, recipient=managers, action_object=organization,
                    verb=f"User {user} has been granted access to organization {organization.code}")
...
def organizationReactivate(request, organizationId):
    ...
    members = User.objects.filter(membership__in=Membership.objects.filter(
        organization=organization, access_granted=True)).exclude(pk=request.user.id)
    notify.send(sender=organization, recipient=members, action_object=organization,
                verb=f"Your organization {organization.code} has been reactivated by {request.user}")
...
def organizationSuspend(request, organizationId):
    ...
    members = User.objects.filter(membership__in=Membership.objects.filter(
        organization=organization, access_granted=True)).exclude(pk=request.user.id)
    notify.send(sender=organization, recipient=members, action_object=organization,
                verb=f"Your organization {organization.code} has been suspended by {request.user}")
