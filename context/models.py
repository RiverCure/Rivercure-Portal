...
class e_ContextDTM(models.Model):
    ...
    contextDTM = models.OneToOneField('raster.RasterLayer', on_delete=models.CASCADE, related_name='dtm_endpoint')
...
class e_ContextBoundaryPoint(models.Model):
    contextBoundaryLine = models.ForeignKey(
        'e_ContextBoundaryLine', on_delete=models.CASCADE, null=True, blank=False, related_name='context_boundary_points')
    # superimposed "must be a point superimposed on e_ContextBoundaryLine.geom"))]
...
class e_ContextSensor(models.Model):
    boundary_point = models.ForeignKey('e_ContextBoundaryPoint', on_delete=models.CASCADE,
                                       null=True, related_name='sensor_boundary_point')
...
class e_ContextEvent(models.Model):
    ...
    UpdateMaximumValueUnit = models.CharField(
        verbose_name='Update maximum value unit', max_length=20, choices=TIME_UNITS, null=True, blank=True)
...
class e_ContextEventResult(models.Model):
    context_event = models.OneToOneField('e_ContextEvent', on_delete=models.CASCADE,
                                         related_name='context_event_results', null=True, blank=True, default=None)
    max_depth = models.OneToOneField('raster.RasterLayer', on_delete=models.CASCADE,
                                     related_name='event_max_depth_result')
    max_level = models.OneToOneField('raster.RasterLayer', on_delete=models.CASCADE,
                                     related_name='event_max_level_result')
