...
var MyFunctions = {
  ...
  drawSensors: (map, sensors) => {
    ...
    for (sensor of sensors) {
      sensorIcon = L.divIcon({
        html: MyFunctions.sensorIcon(sensor.code, 'whitesmoke'),
        className: 'sensor',
        iconSize: [35, 35],
      });
      sensorMarker = L.marker(MyFunctions.coordStringToArray(sensor.geom)[0], {
        icon: sensorIcon,
  ...
  boundaryLinePopupConfig: (popup) => {
    ...
    popup.on('popupopen', (e) => {
      // function to handle the saving of the data
      if (MyFunctions.deleting) {
        //if the user os deleting prevent popup opening
        e.target.closePopup();
        return;
      }
      setTimeout(() => {
        //wait in case user opens popups back to back
        if (!e.popup.isOpen())
  ...
  polygonsPopupConfig: (popup) => {
    popup.on('popupopen', (e) => {
      //define popup alteration saving
      if (MyFunctions.deleting) return;
      ...
      setTimeout(() => {
        ...
        document.querySelector('#popup-btn').addEventListener('click', () => {
          ...
          document.querySelector('#polygon-' + e.target._leaflet_id + '-CL-value').innerHTML =
            '<b>Current CL:</b> ' + document.querySelector('#popup-selected-cl').value;
          e.popup.setContent(
            MyFunctions.polygonsPopup(
              e.target._leaflet_id,
              document.querySelector('#popup-header').innerHTML.split(' ')[0],
              document.querySelector('#popup-selected-cl').value
            )
          );
          e.popup.update();
          setTimeout(() => {
  ...
  sensorAssociationPopup: (id, associatedSensors, newSensor) => {
    ...
    getAssociatedSensors = () => {
      ...
      var result;
      result = associatedSensors.slice(
    ...
    associateNewSensors = () => {
      ...
      if (associatedSensors.lastIndexOf('<th>Sensor ') != -1) {
        nr = associatedSensors.slice(
          associatedSensors.lastIndexOf('<th>') + '<th>'.length,
          associatedSensors.lastIndexOf('</th>')
        );
        nr = Number(nr) + 1;
      }
    ...
    if (MyFunctions.mode == 'edit') {
      return (
        `
                <h1><small id='popup-header'>Water Entry Point ` +
        id +
        `</small></h1>
  ...
  sensorAssociationPopupConfig: (popup) => {
    if (MyFunctions.mode == 'view')
      //if user is in context detail page do nothing
      return;
    ...
    popup.on('popupopen', (e) => {
      ...
      setTimeout(() => {
        ...
        document.querySelector('#popup-btn').addEventListener('click', () => {
          ...
          e.popup.setContent(
            MyFunctions.sensorAssociationPopup(
              e.target._leaflet_id,
              e.popup.getContent(),
        ...
        if (document.querySelector('.close') !== null) {
          document.querySelectorAll('.close').forEach((element) => {
            element.addEventListener('click', (ev) => {
              //button to remove added sensors
  ...
  removeAssociatedSensor: (popup, sensorCode) => {
    let begginning = popup
      .getContent()
      .substring(0, popup.getContent().indexOf("<tr id='".concat(sensorCode)));
    let ending = popup
      .getContent()
      .substr(
        popup
          .getContent()
          .indexOf('</tr>', popup.getContent().indexOf("<tr id='".concat(sensorCode))) +
          '</tr>'.length
      );
    ...
    MyFunctions.contextSensors[code].options.icon.options.html = MyFunctions.sensorIcon(
      code,
      'whitesmoke'
    );
  ...
  overlayOrder: (alignment) => {
    ...
    if (alignment)
      //check if alignment is supposed to be brought to front
    ...
    switch (e.target.value) {
      ...
      case 'Boundary':
        if (MyFunctions.createdPolygons.Domain.getLayers().length === 0) {
          //Domain is not defined therefore disallow user from drawing the boundary
  ...
  mouseMove: (map) => {
    ...
    map.addEventListener('mousemove', (e) => {
      ...
      document.querySelector('#mouse-latlng').innerHTML =
        'Lat: ' + e.latlng.lat.toFixed(5) + ' Lon: ' + e.latlng.lng.toFixed(5);

      if (
        MyFunctions.mode == 'edit' &&
        document.querySelector('#polygon-type').value === 'Boundary' &&
        MyFunctions.boundaryPolyline != null
      ) {
        //if the user is drawing the boundary
  ...
  vertexAdded: (e, map) => {
    ...
    let lat = Object.values(e.layers._layers)[Object.values(e.layers._layers).length - 1]._latlng
      .lat;
    let lng = Object.values(e.layers._layers)[Object.values(e.layers._layers).length - 1]._latlng
      .lng;
    if (polygonType === 'Domain') {
      //if we're drawing the domain then
  ...
  editStop: (map) => {
    ...
    for (coordinate of coordinates) {
      //populate the vertices with markers again
  ...
  addDomainMarker: (map, latLng, sensors) => {
    ...
    MyFunctions.sensorAssociationPopupConfig(
    ...
    if (MyFunctions.mode == 'view')
      //if user is in context detail stop function here
      return;

    //add the logic to draw the boundary based on the domain polygon vertex
    marker.on('click', (e) => {
      if (document.querySelector('#polygon-type').value === 'Boundary') {
        if (MyFunctions.boundaryPolyline === null) {
          //if it's the first point of the polyline being added to the map then
          ...
          //if there are already defined points then draw the polyline
  ...
  stopBoundaryDefinition: () => {
    if (document.querySelector('#polygon-type').value === 'Boundary') {
      //this function is only used when the user is drawing the boundary
      if (
        MyFunctions.boundaryPolyline != null &&
  ...
  drawStart: (e) => {
    ...
    switch (polygonType.value) {
      ...
      case 'Boundary':
        alert(
          'Invalid shape for selected polygon\nClick on the domain markers to draw the boundary'
        );
        //stop drawing
        document.querySelector('a[title="Cancel drawing"]').click();
        break;
      case 'Domain':
        if (MyFunctions.createdPolygons.Domain.getLayers().length >= 1) {
          // allow only 1 domain
  ...
  sendContext: () => {
    ...
    try {
      ...
      domain.properties.CL = popup
        .slice(
          popup.indexOf("current-cl'>") + "current-cl'>".length,
          popup.indexOf("</td id='end-cl")
        )
        .trim();
      ...
      MyFunctions.createdPolygons.Alignment.getLayers().forEach((element) => {
        ...
        alignmentUnit.properties.CL = popup
          .slice(
            popup.indexOf("current-cl'>") + "current-cl'>".length,
            popup.indexOf("</td id='end-cl")
          )
      ...
      MyFunctions.createdPolygons.Refinement.getLayers().forEach((element) => {
        ...
        popup = element.getPopup().getContent(); //get the popup to extract the properties values
        refinementUnit.properties.CL = popup
          .slice(
            popup.indexOf("current-cl'>") + "current-cl'>".length,
            popup.indexOf("</td id='end-cl")
          )
      ...
      MyFunctions.createdPolygons.Boundaries.getLayers().forEach((element) => {
        ...
        boundaryLine.properties.type = popup
          .slice(
        ...
        boundaryLine.properties.dataType = popup
          ...
          .trim();
        for (point of MyFunctions.boundaryDomainMarkers[element._leaflet_id]) {
          //get sensors associated with points
      ...
      document.querySelector('#load-context-result').innerHTML =
        'Unable to save Context<br>Hint: Start by drawing the Domain';
    } finally {
  ...
  pruneDomainMarkers: () => {
    MyFunctions.domainMarkers.eachLayer((marker) => {
      ...
      if (!found) MyFunctions.domainMarkers.removeLayer(marker);
    });
  },
  ...
  clearMap: () => {
    ...
    for (key in MyFunctions.createdPolygons) MyFunctions.createdPolygons[key].clearLayers();
  ...
  drawGeometries: (response, map) => {
    //Domain
    if (response.geomExternalBoundary !== null) {
      //check if the refinement is defined in the database
      let domain = L.polygon(MyFunctions.coordStringToArray(response.geomExternalBoundary), {
        color: '#C0C0C0',
      });
    ...
    if (response.context_boundaries !== null && response.context_boundaries.length > 0) {
      ...
      response.context_boundaries.forEach((element) => {
        //define each boundary line individually
        //Boundary Points
        for (point of element.context_boundary_points) {
          MyFunctions.addDomainMarker(
            map,
            MyFunctions.coordStringToArray(point.geom)[0],
            point.sensor_boundary_point
          );
        }
        //Boundary Lines
  ...
  definePolygon: (layer, type, CL) => {
    var polygonLayer;
    switch (
      type //get the proper storing structure
    ) {
      case 'Domain':
    ...
    if (MyFunctions.mode == 'edit')
      MyFunctions.polygonsPopupConfig(
        layer.bindPopup(MyFunctions.polygonsPopup(L.stamp(layer), type, CL))
      );
    else layer.bindPopup(MyFunctions.polygonsPopup(L.stamp(layer), type, CL));
  ...
  defineBoundary: (boundary, type, dataType) => {
    ...
    for (boundaryLinePoint of boundary.getLatLngs()) {
      for (domainPoint of MyFunctions.domainMarkers.getLayers()) {
        if (boundaryLinePoint.distanceTo(domainPoint.getLatLng()) < 1) {
          //this distance is very sensitive and can cause the duplication of points
  ...
  checkForCompleteness: () => {
    ...
    for (type in MyFunctions.createdPolygons) {
      //check if all polygons are defined
  ...
  handleHighlights: (layer, polygonLayer) => {
    ...
    if (
      polygonLayer === MyFunctions.createdPolygons.Boundaries ||
      polygonLayer === MyFunctions.createdPolygons.Alignment
    )
    ...
    layer.on('mouseout', () => {
      if (MyFunctions.highlightStatus) MyFunctions.highlightLayer.clearLayers();
    });
    layer.on('click', (e) => {
  ...
  highlightPolygon: (polygonId, polygonType, polygonLayer) => {
    if (polygonType)
      L.polyline(polygonLayer.getLayer(polygonId).getLatLngs(), { interactive: false }).addTo(
        MyFunctions.highlightLayer
      );
    else
      L.polygon(polygonLayer.getLayer(polygonId).getLatLngs(), { interactive: false }).addTo(
        MyFunctions.highlightLayer
      );
  },
  ...
  addPolygonToTree: (id, polygon, CL) => {
    ...
    const element = new DOMParser().parseFromString(
      MyFunctions.polygonInTreeHTML(id, polygon, CL),
      'text/html'
    ...
    element.addEventListener('mouseover', () => {
      if (MyFunctions.highlightStatus) {
        MyFunctions.highlightPolygon(id, polygonType, polygonLayer);
      }
    });
    ...
    document.querySelector('#polygon-' + id + '-CL-input-btn').addEventListener('click', () => {
      ...
      document.querySelector('#polygon-' + id + '-CL-value').innerHTML =
        '<b>Current CL:</b> ' + document.querySelector('#polygon-' + id + '-CL-input').value;
      document.querySelector('#polygon-' + id + '-CL-input').value = '';
    });
  },
  ...
  addBoundaryLineToTree: (id, selectedType, selectedDataType) => {
    ...
    element.addEventListener('mouseover', () => {
      if (MyFunctions.highlightStatus) {
        MyFunctions.highlightPolygon(id, true, MyFunctions.createdPolygons.Boundaries);
      }
    });
    element.addEventListener('mouseout', () => {
      if (MyFunctions.highlightStatus) MyFunctions.highlightLayer.clearLayers();
  ...
  addBoundaryPointToTree: (id, addedSensors, boundaryLine) => {
    const element = new DOMParser().parseFromString(
      MyFunctions.boundaryPointInTreeHTML(id, addedSensors),
      'text/html'
    ).firstChild.lastChild.firstChild;
  ...
  polygonInTreeHTML: (id, polygonType, CL) => {
    return (
      ...
      id +
      '-CL-input' +
      ` class="form-control" placeholder="Insert new CL">
  ...
  boundaryPointInTreeHTML: (id, addedSensors) => {
    ...
    buildOptions = () => {
      let addOptions = '';
      let availableSensors = MyFunctions.getNearbySensors(
        addedSensors,
        MyFunctions.domainMarkers.getLayer(id)
      );
    ...
    return (
      `
            <div ` +
      'id=boundarypoint-tree-id-' +
      id +
      ` class="polygon-tree col-md-12 border-top border-bottom">
                <div class="col-md-4">
                    <h1><small>Water Entry Point ` +
      id +
      ` Sensors</small></h1>
                </div>
                <div class="col-md-8">
